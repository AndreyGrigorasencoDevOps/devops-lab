...
- CD runner path uses private DNS plus runner/runtime VNet peering to reach env-local Key Vault private endpoints.
- Dedicated runtime VNets and dedicated CAEs are the repo target for both environments.

Bootstrap note:

- If the Key Vault does not exist yet for an environment, create it first, then add `<env>-db-password`, then run full `plan/apply`.
- Current repo target mode is `firewall`.
- If local bootstrap in `firewall` mode blocks access (`ForbiddenByFirewall`), add a temporary `/32` allowlist rule and pass the same `/32` via `key_vault_allowed_ip_cidrs` during local `plan/apply`, then remove it.
...
- Current repo hardening target is dedicated env Key Vaults with env-local private endpoints + `firewall` mode.
